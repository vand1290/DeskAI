...
/**
 * Represents a complete task chain/workflow
 */
...
/**
 * Result of executing a complete task chain
 */
...
/**
 * Interface for tools that can be used in task chains
 */
...
export class TaskChainManager {
  ...
  private chains: Map<string, TaskChain>;
  private tools: Map<string, ChainTool>;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (fs.existsSync(this.chainsPath)) {
        ...
        const chainsArray: TaskChain[] = JSON.parse(data);
  ...
  private registerDefaultTools(): void {
    ...
    this.registerTool({
      ...
      execute: async (input: unknown, config?: Record<string, unknown>) => {
        ...
        const filename = config?.filename as string || 'output.pdf';
        return {
          status: 'saved',
          filename,
          data: input,
   ...
   */
  async createChain(name: string, description?: string, tags?: string[]): Promise<TaskChain> {
  ...
  async updateStep(
    chainId: string,
    stepId: string,
    updates: Partial<Omit<TaskStep, 'id'>>
  ): Promise<TaskStep> {
    const chain = this.chains.get(chainId);
    if (!chain) {
      throw new Error(`Task chain ${chainId} not found`);
   ...
   */
  async removeStep(chainId: string, stepId: string): Promise<boolean> {
    const chain = this.chains.get(chainId);
    if (!chain) {
      throw new Error(`Task chain ${chainId} not found`);
    }
   ...
   */
  async getChain(chainId: string): Promise<TaskChain | null> {
   ...
   */
  async listChains(tags?: string[]): Promise<TaskChain[]> {
    let chains = Array.from(this.chains.values());

    // Filter by tags if provided
    if (tags && tags.length > 0) {
      chains = chains.filter(chain => 
        chain.tags && chain.tags.some(tag => tags.includes(tag))
      );
    }
   ...
   */
  async executeChain(chainId: string, initialInput?: unknown): Promise<ChainExecutionResult> {
    const chain = this.chains.get(chainId);
    if (!chain) {
      throw new Error(`Task chain ${chainId} not found`);
    }
   ...
   */
  async exportChains(): Promise<TaskChain[]> {
